...
const fetchGithubContents = async (url) => {
  const response = await fetch(url, { cache: 'no-store', headers: GITHUB_API_HEADERS });
...
const loadAlphaDroidFromGithubApi = async (source) => {
  ...
  while (pendingDirs.length > 0) {
    ...
    entries.forEach((entry) => {
      if (entry.type === 'dir' && entry.url) pendingDirs.push(entry.url);
      if (entry.type === 'file' && /\.json$/i.test(entry.name)) jsonEntries.push(entry);
    });
  }
  const settled = await Promise.allSettled(
    jsonEntries.map(async (entry) => {
      const fileResponse = await fetch(entry.download_url, { cache: 'no-store' });
      if (!fileResponse.ok) throw new Error(`File HTTP ${fileResponse.status}`);
      ...
      return parsedDevices.map((device) => ({ ...device, github_file_url: entry.html_url }));
    })
  );
  ...
  const uniqueDevices = Array.from(
    new Map(devices.map((device) => [String(getDeviceCodename(device)).toLowerCase(), device])).values()
  );
...
const loadAlphaDroidFromFallback = async (source) => {
  ...
  return { name: source.name, url: ALPHADROID_FALLBACK_URL, devices };
};

const loadAlphaDroidDevices = async (source) => {
  try { return await loadAlphaDroidFromGithubApi(source); }
  catch (error) { return await loadAlphaDroidFromFallback(source); }
};
...
const loadYaapDevices = async (source) => {
  ...
  const settled = await Promise.allSettled(
    jsonEntries.map(async (entry) => {
      ...
      return parsedDevices.length > 0
        ? parsedDevices.map((device) => ({
            codename: device.codename || device.device || device.id || fallbackCodename,
            name: device.device_name || device.name || device.model || fallbackCodename,
            ...device,
          }))
        : [{ codename: fallbackCodename, name: fallbackCodename }];
    })
  );
  const devices = settled
    .filter((result) => result.status === 'fulfilled')
...
const getDeviceBrand = (device) =>
  device.brand || device.manufacturer || device.oem || device.vendor || '';
...
const buildDownloadUrl = ({ romName, codename, device }) => {
  if (romName === 'LineageOS') {
    return codename && codename !== 'unknown' 
      ? `https://download.lineageos.org/devices/${encodeURIComponent(codename)}/builds` 
  ...
  const directUrl = device.download_url || device.downloadUrl || device.url || device.rom_url || device.website || device.link || (romName === 'AlphaDroid' ? device.github_file_url : null);
...
const renderRomCard = ({ name, url, devices, error }) => {
  ...
  if (error) {
    meta.textContent = 'Failed to fetch device list';
    errorText.hidden = false;
    errorText.textContent = error;
    card.dataset.state = 'error';
    return node;
  }
  meta.textContent = `${devices.length} devices found`;
  devices
  ...
  if (devices.length === 0) {
    const li = document.createElement('li');
    li.textContent = 'No devices found for this source.';
...
const loadSource = async (source) => {
  ...
  try {
    if (type === 'alphadroid-repo') return await loadAlphaDroidDevices(source);
    if (type === 'yaap-repo') return await loadYaapDevices(source);
    const response = await fetch(url, { cache: 'no-store' });
    if (!response.ok) throw new Error(`HTTP ${response.status}`);
    const payload = await response.json();
    return { name, url, devices: normalizeDevices(payload) };
  } catch (error) {
    return { name, url, devices: [], error: 'Data fetch error. The source URL might be incorrect or restricted by CORS.' };
  }
};
...
const refreshData = async () => {
  ...
  results.forEach((result) => { romGrid.append(renderRomCard(result)); });
  lastUpdated.textContent = `Last updated: ${new Date().toLocaleString('en-US')}`;
  romCountBadge.textContent = `${ROM_SOURCES.length} Sources`;
